...
- 本リポジトリ／本コードは **あくまで参考（雛形・サンプル）** です。実運用への適用を保証するものではありません。
- **プロジェクトごとに要件・制約（権限、ネットワーク、命名、監査、SLO など）が異なる** ため、そのまま適用せず、必ず内容を確認した上で **各プロジェクトに合わせて修正** してください。
- 本コードの利用により発生したいかなる損害についても、作成者は **責任を負いません**（自己責任でご利用ください）。
